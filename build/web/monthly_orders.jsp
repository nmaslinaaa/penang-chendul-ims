                      ...
                      <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                          <c:forEach var="order" items="${orders}" varStatus="status">
                            <c:set var="previousOrderId" scope="page"><c:out value="${pageScope.previousOrderId}"/></c:set>
                            <c:if test="${order.orderId != previousOrderId}">
                              <c:set var="orderCount" value="1" scope="page"/>
                              <c:set var="previousOrderId" value="${order.orderId}" scope="page"/>
                            </c:if>
                            <tr>
                              <td class="text-center"><c:out value="OR00${order.orderId}"/></td>
                              <td class="text-center"><c:out value="${pageScope.orderCount}"/></td>
                            ...
                            <c:set var="orderCount" value="${pageScope.orderCount + 1}" scope="page"/>
                          </c:forEach>
                        </tbody>
                      </table>
                    </div>
